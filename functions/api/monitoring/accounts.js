...
import { createMonitoringAccount, createMonitoringAccountSchema } from "../../_lib/monitor.js";
...
export const onRequestPost = async (context) => {
  try {
    ...
    const parsed = createMonitoringAccountSchema.safeParse(payload);
    if (!parsed.success) {
      return json({ ok: false, message: "Charge utile invalide.", issues: parsed.error.issues }, { status: 400 });
    }

    const result = await createMonitoringAccount(context.env, parsed.data);
    return json({ ok: true, ...result }, { status: 201 });
  } catch (error) {
    return json({ ok: false, message: error.message || "Création de compte Monitor impossible." }, { status: 400 });
  }
};
